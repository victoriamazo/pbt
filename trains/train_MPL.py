...
class train_MPL(Train):
    def __init__(self, FLAGS):
        ...
        if hasattr(FLAGS, 'load_ckpt_iter') and FLAGS.load_ckpt_iter != '':
            self.load_ckpt_iter = FLAGS.load_ckpt_iter
    ...
    def build(self):
        ...
        if self.load_ckpt_iter != '':
            model_path = os.path.join(self.ckpts_dir, '{}.pth'.format(self.load_ckpt_iter))
        ...
        try:
            ...
            for iteration, (data, target, _) in enumerate(train_loader):
                ...
                if iteration%10 == 0:
                    print('epoch {} (iter {})'.format(epoch, iteration))  #, ' train_iter = ', train_iter
